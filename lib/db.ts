...
async function readDb() {
  ...
  if (!db.data) {
    ...
    // 确保必要的字段存在
    if (!Array.isArray(db.data.categories)) db.data.categories = [...defaultData.categories]
...
// 更新类型
export async function updateType(oldType: string, newType: string): Promise<string[]> {
  const data = await readDb()
...
// 添加主机组
export async function addHostGroup(group: Omit<HostGroup, "id" | "lastUpdated">): Promise<HostGroup[]> {
  const data = await readDb()
...
// 更新主机组
export async function updateHostGroup(category: string, group: Partial<HostGroup>): Promise<HostGroup[]> {
  const data = await readDb()
...
// 添加主机
export async function addHost(category: string, host: Omit<HostChild, "id" | "lastUpdated">): Promise<HostGroup[]> {
  const data = await readDb()
...
// 更新主机 - 修改为支持跨部门移动
export async function updateHost(category: string, hostId: string, host: Partial<HostChild>): Promise<HostGroup[]> {
  const data = await readDb()
  ...
  const hostIndex = data.hosts[groupIndex].children.findIndex((h) => h.id === hostId)
  ...
  if (host.category && host.category !== category) {
    ...
    // 查找新部门索引
    let newGroupIndex = data.hosts.findIndex((h) => h.category === host.category)
    ...
    // 从原部门删除
    data.hosts[groupIndex].children = data.hosts[groupIndex].children.filter((h) => h.id !== hostId)
...
// 删除主机
export async function deleteHost(category: string, hostId: string): Promise<HostGroup[]> {
  const data = await readDb()
  const groupIndex = data.hosts.findIndex((h) => h.category === category)
